...
Purely about talking to the server - it doesn't know what a ``ToolService``
is. ``ToolService.register_mcp`` (see ``tool/service.py``) is what takes
...
from tool.service import RegisteredTool
    ...
    def _adapt(self, remote_tool: Tool) -> RegisteredTool:
        ...
        async def handler(**arguments: object) -> object:
            ...
            if result.is_error:
                ...
                # ToolService.call_tool already turns a raised exception

...
* [ ] **Agent definition caching**
  ...
  * Cache agents by `owner_id` in Redis to avoid querying MongoDB on every agent switch/request.
  * Cache both:

    * `findMany(owner_id)`
    * `findOne(id, owner_id)`
  * Add a configurable TTL.
  * Define clear cache keys, e.g.:

    * `agents:{owner_id}`
    * `agent:{owner_id}:{agent_id}`
  * Review whether one owner-level cache is sufficient instead of duplicating individual agent caches.
  * Measure/verify that switching agents no longer causes unnecessary MongoDB reads.

* [ ] **Incremental agent-version updates**

  * When an agent definition is updated and its version changes, update/invalidate the corresponding Redis cache immediately.
  * MongoDB remains the source of persistence; Redis is the read cache.
  * Ensure the new version replaces the old cached definition atomically.
  * Avoid serving an older agent definition after an update.
  * Consider cache invalidation/update strategy for:

    * single-agent cache
    * owner-agent-list cache
    * compiled runtime cache
  * Verify behavior across multiple application instances.

* [ ] **Review `agent_session_lock`**

  * Explain exactly **when and why** the Redis session lock is acquired.
  * Document the complete flow:
    `request → acquire lock → load session → run agent → save session → release lock`
  * Explain what race condition it prevents when two requests use the same `session_id`.
  * Determine whether the current distributed Redis lock is the best solution.
  * Compare alternatives such as:
    ...
    * queue-based per-session execution
  * Keep the simplest solution that provides correct session consistency.

* [ ] **Simplify Python architecture/code style**

  * Review the entire codebase for unnecessarily complicated or overly "Python-framework-like" implementations.
  * Simplify modules while preserving the existing modular architecture and SOLID principles.
  * Prefer the conventions and ergonomics of **NestJS** where they make sense:
    ...
    * straightforward providers/adapters
    * minimal boilerplate
  * Remove unnecessary abstractions, wrappers, factories, indirections, and duplicated logic.
  * Keep **modular clean architecture** boundaries intact.
  * Prioritize code that is short, readable, testable, and easy for a NestJS/TypeScript engineer to understand.
  * **Do not over-engineer.**

* [ ] **Swagger UI**

  * Add Swagger/OpenAPI UI support.
  * Install the appropriate FastAPI/OpenAPI tooling/library if required.
  * Expose interactive API documentation.
  * Ensure all public `/agents` endpoints have useful request/response schemas and descriptions.
  * Verify Swagger UI works with the complete application.

* [ ] **Duplicate-message handling / rate limiting**

  * When the same user/session sends two messages close together, make handling smarter.
  * Investigate request deduplication/idempotency before simply rejecting requests.
  * Consider:

    * short-lived Redis request/message cache
    * idempotency keys
    * request fingerprinting
    * per-session rate limiting
    * existing session locking
  * Avoid executing the same expensive LLM workflow twice unnecessarily.
  * Preserve legitimate consecutive messages.
  * Define appropriate TTLs and behavior for duplicate vs. genuinely new requests.

* [ ] **LLM performance and model review**

  * Review the current LLM configuration and optimize for **both latency and answer/tool-calling quality**.
  * Consider the available RAM and existing locally installed models before recommending new downloads.
  * Benchmark suitable models against:
    ...
    * reasoning quality
    * memory usage
  * Review Ollama configuration, model keep-alive, context size, reasoning settings, and generation limits.
  * Avoid blindly switching to a larger model if it materially hurts latency on the available hardware.
  * Prefer the best practical model/configuration for this machine rather than theoretical model quality. 

### Important implementation principle

For **all seven changes**, first inspect the existing implementation and then make the **smallest architectural change necessary**. The project explicitly favors avoiding speculative abstractions and over-engineering. 
